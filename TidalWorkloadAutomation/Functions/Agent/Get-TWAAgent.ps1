Function Get-TWAAgent {
    <#
    .SYNOPSIS
    Get all Agents or specific Agent information based on Type.

    .DESCRIPTION
    return Agent information based on input paramters

    .EXAMPLE
    Get-TWAAgent -Type [ AgentList | AgentListJoin | WindowsAgentList | UnixAgentList | ServiceAgentList | OVMSAgentList | MVSAgentList | OwnerAgent ] 
    Get-TWAAgent -Type AgentList -AgentID 20, 40, 1002 
    Get-TWAAgent -Type AgentList -Search '*agentname*' -Limit 3 
    (Get-TWAAgent -Type AgentListJoin).agentlistjoin | Where-Object {$_.agentlistid -eq (Get-TWAAgent -Type AgentList -AgentID 15).agentlist.id}
    ...
    param (
        ...
        [ValidateSet("AgentList","AgentListJoin","WindowsAgentList","UnixAgentList","ServiceAgentList","OVMSAgentList","MVSAgentList","OwnerAgent")]
        [string]$Type,
