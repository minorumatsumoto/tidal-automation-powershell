Function Get-TWAJobByType {
    <#
    .SYNOPSIS
    Get all Jobs or specific Job information for given type (e.g. FTPJob).

    .DESCRIPTION
    return job information based on a given type

    .EXAMPLE
    Get-TWAJobRun -Type [ JobGroup | DataMoverJob | EventsJob | FTPJob | MPEJob | OS400Job | OSJob | OVMSBatchJob | OVMSCommandJob | OVMSJob | ServiceJob ]
    Get-TWAJobRun -Type FTPJob -JobTypeID 47283, 47284, 47288
    Get-TWAJobRun -Type FTPJob -Search '*audit*' -Limit 1
    ...
    param (
        ...
        [ValidateSet("JobGroup","DataMoverJob","FTPJob","MPEJob","OS400Job","OSJob","OVMSBatchJob","OVMSCommandJob","OVMSJob","ServiceJob")]
        [string]$Type,
