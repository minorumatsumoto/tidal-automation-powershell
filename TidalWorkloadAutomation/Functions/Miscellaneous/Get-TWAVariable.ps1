Function Get-TWAVariable {
    ...
    .EXAMPLE
    Get-TWAVariable
    Get-TWAVariable -VarID 7, 10, 11
    Get-TWAVariable -Search '*varname*' -Limit 6
    ...
    process	
    {
        ...
        if ($VarID)
        {
            foreach ($id in $VarID)
            {
                $Body   = Set-RestAPIObjectRetrieve -id $id -tesParam "Variable.get"
        ...
        elseif ($Search) 
        {
            ...
            $Body   = Set-RestAPIObjectRetrieve -tesParam "Variable.getList"
            $result = Invoke-TWARestMethod -RestURL $TWAURL -Method "POST" -Body $Body -Header $Header
