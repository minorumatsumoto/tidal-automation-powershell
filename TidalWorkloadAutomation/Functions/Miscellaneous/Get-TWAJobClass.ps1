Function Get-TWAJobClass {
    <#
    .SYNOPSIS
    Get all Tidal variables or specific variable information.

    .DESCRIPTION
    return Tidal variable information based on input paramters

    .EXAMPLE
    Get-TWAJobClass
    Get-TWAJobClass -JobClassID 7, 10, 11
    Get-TWAJobClass -Search '*varname*' -Limit 6
    ...
    process	
    {
        ...
        if ($JobClassID)
        {
            foreach ($id in $JobClassID)
            {
                $Body   = Set-RestAPIObjectRetrieve -id $id -tesParam "JobClass.get"
        ...
        elseif ($Search) 
        {
            ...
            $Body   = Set-RestAPIObjectRetrieve -tesParam "JobClass.getList"
            $result = Invoke-TWARestMethod -RestURL $TWAURL -Method "POST" -Body $Body -Header $Header
