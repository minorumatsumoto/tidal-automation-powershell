Function Get-TWAQueue {
    <#
    .SYNOPSIS
    Get information about a queues status.

    .DESCRIPTION
    return Tidal queue information based on input paramters

    .EXAMPLE
    Get-TWAQueue
    Get-TWAQueue -QueueID 3, 5, 8
    Get-TWAQueue -Search '*failed*' -Limit 6
    ...
    process	
    {
        ...
        if ($QueueID)
        {
            foreach ($id in $QueueID)
            {
                $Body   = Set-RestAPIObjectRetrieve -id $id -tesParam "Queue.get"
        ...
        elseif ($Search) 
        {
            ...
            $Body   = Set-RestAPIObjectRetrieve -tesParam "Queue.getList"
            $result = Invoke-TWARestMethod -RestURL $TWAURL -Method "POST" -Body $Body -Header $Header
