Function Get-TWAAlert {
    <#
    .SYNOPSIS
    Get all Tidal alerts or specific alert information.

    .DESCRIPTION
    return Tidal alert information based on input paramters

    .EXAMPLE
    Get-TWAAlert
    Get-TWAAlert -AlertID 3, 5, 8
    Get-TWAAlert -Search '*failed*' -Limit 6
    ...
    process	
    {
        ...
        if ($AlertID)
        {
            ...
            {
                $Body   = Set-RestAPIObjectRetrieve -id $id -tesParam "Alert.get"
        ...
        elseif ($Search) 
        {
            ...
            $Body   = Set-RestAPIObjectRetrieve -tesParam "Alert.getList"
            $result = Invoke-TWARestMethod -RestURL $TWAURL -Method "POST" -Body $Body -Header $Header
