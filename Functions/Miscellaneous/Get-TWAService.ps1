Function Get-TWAService {
    <#
    .SYNOPSIS
    Get all Service or specific Service information.

    .DESCRIPTION
    return Tidal Service information based on input paramters

    .EXAMPLE
    Get-TWAService
    Get-TWAService -ID 7, 10, 11
    Get-TWAService -Search 'id' -Limit 6
    ...
    process	
    {
        ...
        if ($ID)
        {
            foreach ($id in $ID)
            {
                $Body   = Set-RestAPIObjectRetrieve -id $id -tesParam "Service.get"
        ...
        elseif ($Search) 
        {
            ...
            $Body   = Set-RestAPIObjectRetrieve -tesParam "Service.getList"
            $result = Invoke-TWARestMethod -RestURL $TWAURL -Method "POST" -Body $Body -Header $Header
